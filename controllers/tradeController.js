...
// Show all trades with their categories
exports.trades = (req, res, next) => {

    items = itemModel.find()
        .then(response => {
            const categories = itemModel.distinct("category")
                .then(r => {
...
exports.trade = (req, res, next) => {
    ...
    itemModel.findById(id).populate('owner', 'firstName lastName')
        .then(item => {
            if (item) {                
...
exports.addWatchList = (req, res, next) => {
    ...
    itemModel.findById(id)
        .then(item => {
            if (item) {
                item.watchlist = 'Unwatch'
                item.watchListBy.push(userId)

                itemModel.findByIdAndUpdate(id, item, { useFindAndModify: false, runValidators: true })
                    .then(item => {
                        if (item) {
                            req.flash('success', 'Item added to watchlist. Check your watchlist on profile');
...
exports.removeWatchList = (req, res, next) => {
        ...
        itemModel.findById(id)
            .then(item => {
                if (item) {
                    item.watchlist = 'Watch'

                    var index = item.watchListBy.indexOf(userId);
                   
                    if(index > -1){
                        item.watchListBy.splice(index,1);
                    }
                 
                    itemModel.findByIdAndUpdate(id, item, { useFindAndModify: false, runValidators: true })
                        .then(item => {
                            if (item) {
                                req.flash('success', 'Item removed from watchlist. Check your watchlist on profile');
                                res.redirect('/trades/' + id);
        ...
        itemModel.findById(tradeItem)
            .then(item => {
                if (item) {
                    ...
                    itemModel.findByIdAndUpdate(tradeItem, item, { useFindAndModify: false, runValidators: true })
                        .then(item => {
                            if (item) {
                                req.flash('success', 'Which your item you want to trade?');
                                res.render('./trades/tradeIt', {itemData, selectedTradeItem})                     
                            } else {
                                let err = new Error('Cannot find a trade with id ' + tradeItem);
                                err.status = 404;
                                next(err);
                            }
                        })
...
exports.tradeSelected = (req,res,next) => {
    ...
    itemModel.findById(userItemID)
        .then(item => {
            if (item) {
                item.trade.tradeItemID = tradeItemID
                item.status = 'Offer Pending'
                itemModel.findByIdAndUpdate(userItemID, item, { useFindAndModify: false, runValidators: true })
                    .then(item => {
                        if (item) {
                            itemModel.findById(tradeItemID)
                            .then(tradeItemResponse => {
                                tradeItemResponse.trade.myItemID = userItemID
                                tradeItemResponse.trade.tradeItemID = tradeItemID

                                itemModel.findByIdAndUpdate(tradeItemID, tradeItemResponse, { useFindAndModify: false, runValidators: true })
                                .then(result=>{
                                    req.flash('success', 'Offer is made. You can manage your offer on your profile.');
                                    res.redirect('/trades');
                                })
                                .catch(err => next(err));
                            })
                        ...
                        if (item) {
                            ...
                            if(userItemID !=0){
                                ...
                                .then(userItemInfo => {
                                    ...
                                    itemModel.findByIdAndUpdate(userItemID,userItemInfo, { useFindAndModify: false, runValidators: true })
                                    .then(userData=>{
            ...
            if (allItems) {
                    ...
                    allItems.forEach(tradeitem => {
                        if(tradeitem.trade.myItemID == myItemid){
                            if(myItemid !=0){
                                const myItem = itemModel.findById(myItemid)
                                .then(myItemInfo=> {
                                    req.flash('success', 'Are you sure about this trade?');
                                    return res.render('./trades/confirmTrade',{tradeitem, myItem: myItemInfo,userId});                              
                                })
                                .catch(err => next(err));  
                            }                                                    
                        }
                            ...
                            if(myItemid !=0){
                                const myItemData = itemModel.findById(myItemid)
                                .then(myItemDataRes => {
                                    if(myItemDataRes.trade.myItemID != 0){
                                        const tradeItemData = itemModel.findById(myItemDataRes.trade.myItemID)
                                        .then(tradeItemDataRes => {
                                            req.flash('success', 'Are you sure about this trade?');
                                            return res.render('./trades/confirmTrade',{tradeitem : tradeItemDataRes, myItem: myItemDataRes,userId});
                                        })
                                        .catch(err => next(err));
                                    }
...
exports.acceptOffer = (req,res, next) => {
    ...
    .then(tradeItemData => {
        ...
        itemModel.findByIdAndUpdate(tradeItemId,tradeItemData, { useFindAndModify: false, runValidators: true })
        .then(tradeItemRes=> {

            itemModel.findById(myItemId)
            .then(myItemData => {
                myItemData.status = 'Traded';
                myItemData.trade.status = 'Completed';

                itemModel.findByIdAndUpdate(myItemId,myItemData,{ useFindAndModify: false, runValidators: true })
                .then(myItemRes => {
                    req.flash('success', 'Congratulations!!!! Your trade is completed');
                    res.redirect('/trades');
                })
                .catch(err => next(err));    
            })
